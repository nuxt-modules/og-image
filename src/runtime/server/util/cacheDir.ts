...
export function resolveCacheDir(options: { packageName?: string, createIfMissing?: boolean, rootDir?: string } = {}): string | null {
  ...
  // Use provided rootDir or try to get it from runtime config
  let startDir: string
  if (options.rootDir) {
    startDir = options.rootDir
  }
  else {
    try {
      // Try to get rootDir from runtime config
      const config = useRuntimeConfig()
      startDir = (config.app?.rootDir as string) || '.'
    }
    catch (e) {
      // Fallback to current directory if runtime config is not available
      startDir = '.'
    }
  }
