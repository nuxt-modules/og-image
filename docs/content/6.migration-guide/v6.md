...
### Object Syntax with `url` Replaced by `defineOgImageUrl`

The `defineOgImage()`{lang="ts"} object syntax for pre-prepared images has been replaced with a dedicated composable.

```diff
- defineOgImage({ url: '/my-image.png', width: 1200, height: 600, alt: 'My Image' })
+ defineOgImageUrl('/my-image.png', { width: 1200, height: 600, alt: 'My Image' })
```

See the [`defineOgImageUrl()`{lang="ts"}](/docs/og-image/api/define-og-image-url) API reference for full usage.
...
If you have any hardcoded references to the old URL format, update them to use `getOgImagePath()`{lang="ts"} which is auto-imported:
...
Note that `defineOgImage()`{lang="ts"} now returns an array of generated paths, so you may not need `getOgImagePath()`{lang="ts"} at all:
