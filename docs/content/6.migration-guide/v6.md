...
**v6 format:**
```txt
/_og/d/w_1200,title_Hello+World,p_Ii9zYXRvcmki.png
```
...
When prerendering, the module switches to hash mode in two cases. The first is an encoded path over 200 characters; the filesystem limit is 255. The second is a path that holds a character outside `A-Z a-z 0-9 _ . ~ -`, such as a comma or a plus sign. Some static hosts redirect those paths, and social crawlers often do not follow the redirect.
...
::tip
See the [Performance](/docs/og-image/guides/performance) guide for strategies to reduce URL size, optimize component rendering, and configure caching for production.
::
...
See [PR #410](https://github.com/nuxt-modules/og-image/pull/410). This option was deprecated and has no effect in Nuxt Content v3. Remove it from your config:
...
See [PR #410](https://github.com/nuxt-modules/og-image/pull/410). If you were importing from this legacy alias, update your imports:
