...
When calling `defineOgImage({ component: 'MyTemplate' })`{lang="ts"}, the module uses the first registered variant. To select a specific renderer, use dot notation (`'MyTemplate.takumi'`) or PascalCase (`'MyTemplateTakumi'`).
