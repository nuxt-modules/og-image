...
- `siteUrl` - You must migrate to using [Nuxt Site Config](/docs/site-config/guides/setting-site-config)
...
If you need to use components, do a search for these components and replace any instances or variations with
`<OgImage ... />`{lang="html"}
...
The props work a bit differently. You will need to pass them in explicitly as a second argument.
