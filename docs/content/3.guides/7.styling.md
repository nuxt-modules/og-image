...
The module extracts classes from your OG components at build time and compiles them with Tailwind's CSS engine.

### Theme Customizations

The module automatically resolves custom theme values from your CSS:
...
If using Tailwind v4 with the [Vite](https://vite.dev) plugin (not `@nuxtjs/tailwindcss`), point the module to your CSS:
...
If using `@nuxtjs/tailwindcss`, the module auto-detects your setup - zero-config.

## Inline Styles

Utility classes may not be enough to style your templates in some cases.
...
You can use `<style>`{lang="html"} tags within your templates, however, it requires the `inline-css` dependency, which has limited
