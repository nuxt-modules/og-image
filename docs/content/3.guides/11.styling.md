...
relatedPages:
  - path: /docs/og-image/integrations/color-mode
    title: Nuxt Color Mode
  - path: /docs/og-image/guides/icons-and-images
    title: Icons and Images
  - path: /docs/og-image/renderers/satori
...
**Satori constraints:** the [Satori](/docs/og-image/renderers/satori) renderer treats everything as a flexbox with `flex-direction: column` by default. It lacks support for `display: grid`, `display: block`, and `display: inline`.

**Takumi & Browser:** the recommended [Takumi](/docs/og-image/renderers/takumi) and [Browser](/docs/og-image/renderers/browser) renderers offer much broader CSS support, including CSS Grid and standard block/inline layouts.
