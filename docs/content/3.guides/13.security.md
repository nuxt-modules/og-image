...
When URL signing is active, `maxQueryParamSize` has no effect because query parameters are already ignored entirely. All other security options (`maxDimension`, `renderTimeout`, `restrictRuntimeImagesToOrigin`) continue to apply as defense-in-depth.
...
::note
This option has no effect when `security.secret` is configured, since query parameters are ignored when URL signing is active.
::
