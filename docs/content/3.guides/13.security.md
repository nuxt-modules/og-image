...
Load custom fonts via [`@nuxt/fonts`](https://fonts.nuxt.com); it downloads them at build time and serves them same-origin. The deprecated `defineOgImage({ fonts })`{lang="ts"} option only loads fonts from **same-origin** paths, because the `fonts` URL parameter is attacker-controlled and an arbitrary external URL would let the font loader act as a server-side request forgery (SSRF) relay into your internal network (GHSA-q8hw-4fvp-9rwv).

A font path is accepted when it is:

- a relative path (e.g. `/fonts/MyFont.ttf`), or
- an absolute URL whose origin matches your [site URL](https://nuxtseo.com/docs/site-config/getting-started/introduction) (requires the site URL to be configured), or
- an inline `data:` URI.

Any other (cross-origin) font URL is ignored. Same-origin fetches are still blocked from resolving to private, loopback, or link-local addresses, and redirects are re-validated on every hop.
