...
Fonts are normally served same-origin (e.g. `/_fonts/...` from `@nuxt/fonts`), and those paths are always allowed. Because the `fonts` URL parameter is attacker-controlled, **external (non-same-origin) font URLs are denied by default** so the font loader cannot act as a server-side request forgery (SSRF) relay into your internal network (GHSA-q8hw-4fvp-9rwv).
