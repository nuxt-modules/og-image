...
The cache key is deterministic from the path alone. Appending query parameters like `?ref=twitter` or `?utm_source=og` to an OG image URL has no effect. If you need different image variants, use a [custom cache key](#custom-cache-key).

Your page URL's query parameters (e.g., `/products?page=2`) also do not affect OG image caching. Each page's `defineOgImage` call generates a unique OG image URL with different path-encoded options.
