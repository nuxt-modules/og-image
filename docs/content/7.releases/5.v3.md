...
relatedPages:
  - path: /docs/og-image/migration-guide/v3
    title: v2 to v3
  - path: /docs/og-image/releases/v4
    title: v4.0.0
  - path: /docs/og-image/releases/v2
    title: v2.0.0
---

## Background

I'm excited to share that v3 of Nuxt OG Image is here.
...
The default design view. This is similar to the v2 playground. It gives you HMR of the template, the ability
to change the options used to render the template.

New features include social media previews (Twitter, Slack), changing
the rendering (Browser, Satori) and more.

::tab-comparison

:FigureImage{class="m-0" src="https://github.com/harlan-zw/nuxt-og-image/assets/5326365/e337b490-dccb-4e58-972a-5e6e63f30986" alt="v3 preview of OG Image"}
:FigureImage{class="m-0" src="https://private-user-images.githubusercontent.com/5326365/250609887-37482fa1-2f1b-430c-8c20-2f5095dfae56.png?jwt=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..0XUVBDEuA1TROR056mKcwc1LFcJsnA0wzST_nUz0vGE" alt="v2 preview of OG Image"}
::
...
:FigureImage{src="https://github.com/harlan-zw/nuxt-og-image/assets/5326365/ab0f8d86-3167-4369-af2b-dfa4153a08ea" alt="v3 gallery tab"}
- **Debug**
...
Satori itself provides a subset of [Tailwind CSS](https://tailwindcss.com) classes that you can use to style your templates. This is great but also
limited.
...
Using a component or composable within your markdown content is awkward.
...
With the new route rule support, you can now handle this even more easily.
...
Whenever a template has a `<style>`{lang="html"} block, we need to use the [css-inline](https://github.com/Stranger6667/css-inline) dependency to make sure
the styles get applied correctly.

This dependency previously only had compatibility for Node based environments. With the work the library has done
...
In v2 we render OG Images at 1200x630 (1.91:1). This was the recommended resolution from Facebook.

For better compatibility with other social platforms, we now use 1200x600 (2:1). This size will render OG Images better on WhatsApp.
...
Now you can use the `fonts` prop on the `defineOgImage` component.
...
:FigureImage{src="/_og/d/og-image/releases/v3/og.png?component=Release&theme=%23d946ef&siteName=Purple&icon=carbon%3Afire" alt="v3 dark mode"}
:FigureImage{src="/_og/d/og-image/releases/v3/og.png?component=Release&colorMode=light" alt="v3 light mode"}
:FigureImage{src="/_og/d/og-image/releases/v3/og.png?component=Release&colorMode=dark" alt="v3 dark mode"}
::
...
While caching for v2 worked, it was a mess with many configuration options.
...
This also means that if you're generating your app with `nuxi generate`, the module will no longer bundle
any Satori or [Playwright](https://playwright.dev) dependencies.
...
### Opting out of OG Images

On some pages, you may want to disable the OG Image generation. You can now do this by providing a false
