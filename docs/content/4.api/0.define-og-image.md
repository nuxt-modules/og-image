...
It renders a custom OG image from a Vue component. To use a pre-prepared image instead, see [`defineOgImageUrl()`{lang="ts"}](/docs/og-image/api/define-og-image-url).
...
To use a pre-prepared image instead of generating one, use [`defineOgImageUrl()`{lang="ts"}](/docs/og-image/api/define-og-image-url):
