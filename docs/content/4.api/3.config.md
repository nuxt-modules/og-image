...
- Type: `{ maxDimension?: number, maxDpr?: number, renderTimeout?: number }`{lang="ts"}
- Default: `{ maxDimension: 2048, maxDpr: 2, renderTimeout: 10000 }`{lang="ts"}

Security limits for image generation. Prevents denial of service via oversized dimensions, unbounded DPR, or long running renders.

- **`maxDimension`**: Maximum width or height in pixels. Both values are clamped after all option sources merge. Default `2048`.
- **`maxDpr`**: Maximum device pixel ratio for the Takumi renderer. Default `2`.
- **`renderTimeout`**: Milliseconds before the render is aborted with a `408` response. Default `10000`.

Loopback and private IP URLs in `<img>`{lang="html"} and `background-image` are also blocked outside of dev mode.
