...
This page documents all available configuration options for the Nuxt OG Image module. These options allow you to customize how OG images are generated, cached, and rendered across your entire application.

Most applications will work fine with the default configuration. Only modify these settings when you need to:
- Change default image dimensions or appearance
- Use custom fonts beyond the default Inter font
- Configure caching behavior for production
- Integrate with specific hosting providers
- Optimize for non-English content

For page-specific OG image customization, see the [defineOgImage API](/docs/og-image/api/define-og-image) instead.
...
Customize default appearance across all OG images:
...
Configuration for production with caching and prerendering:
...
Disable in production to use static social images:
