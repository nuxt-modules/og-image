...
- Type: `number`{lang="ts"}
- Default: `259200`{lang="ts"} (3 days)

Default cache duration in seconds for generated OG images. Controls the internal storage TTL, HTTP `Cache-Control` headers, and the auto-configured SWR route rule.

Can be overridden per-image via the `cacheMaxAgeSeconds` option in `defineOgImage`.
