...
By default, the module version is used which invalidates the cache on module upgrades. Set a static value like `'v1'` to persist the cache across module updates. Set to `false` to disable versioning entirely.
...
This option only controls which subsets are downloaded when supplemented missing families for the [Satori](/docs/og-image/renderers/satori) renderer. Fonts provided via `@nuxt/fonts` are always included with all their available subsets.
...
When enabled, requests like `/page?foo=bar` will have a separate cache from `/page`. Enable this if your OG image content depends on query parameters.
