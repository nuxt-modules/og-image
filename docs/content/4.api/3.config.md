...
- **`fontHostAllowlist`**: Hosts allowed for external (non-same-origin) font URLs passed via `defineOgImage({ fonts: [{ path }] })`{lang="ts"}. External font hosts are default-denied to prevent SSRF; same-origin font paths (e.g. `/_fonts/...` from `@nuxt/fonts`) are always allowed. Add a host to fetch fonts from a specific external CDN, e.g. `['fonts.gstatic.com']`. Default `[]`.
