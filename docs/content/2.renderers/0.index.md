...
components/OgImage/
  ...
  Screenshot.chromium.vue  # Chromium renderer
```
...
::warning
**Do not mix renderers for the same component name.**

For example, do NOT create both `MyTemplate.satori.vue` and `MyTemplate.chromium.vue`. The module cannot reliably resolve which one to use when you call `defineOgImage('MyTemplate')`.

If you need different renderers, use unique component names (e.g., `MyTemplateSatori` and `MyTemplateChromium`).
::
