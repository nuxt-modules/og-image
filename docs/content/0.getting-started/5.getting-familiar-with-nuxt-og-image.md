...
This will use the default template [NuxtSeo](/docs/og-image/api/nuxt-seo-template) with the [Takumi](/docs/og-image/renderers/takumi) renderer.
...
Any components you add to an `OgImage` folder will be automatically registered as templates for you. The `.takumi` suffix tells the module which renderer to use — [Takumi](/docs/og-image/renderers/takumi) is the recommended renderer.
...
Tailwind v4, UnoCSS, CSS variables, and Nuxt UI v3 colors are all supported out of the box. You can learn more in the [Styling](/docs/og-image/guides/styling) guide.

::warning{icon="i-heroicons-exclamation-triangle"}
**CSS classes are processed at build time.** Dynamic class names like `` :class="[`border-${color}`]" `` won't work because the generated class isn't in the stylesheet. Use static classes for Tailwind/UnoCSS, and `:style` bindings for dynamic values.
::
...
Since CSS classes are processed at build time, we use a `:style` binding for the dynamic border color
